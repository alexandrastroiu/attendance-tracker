...
const TeacherMarkAttendance = () => {
  ...
  const handleSelectCourse = (course_id) => {
    ...
    fetch(
      `http://localhost:8888/management_attendance/attendance-tracker/backend/api/teacher/getSessions.php?course_id=${course_id}`,
      {
        credentials: "include",
      }
    )
  ...
  const handleSelectSession = (session_id) => {
    ...
    fetch(
      `http://localhost:8888/management_attendance/attendance-tracker/backend/api/teacher/getSessionStudents.php?session_id=${session_id}`,
      {
        credentials: "include",
      }
    )
      ...
      .then((data) => {
        if (Array.isArray(data)) {
          ...
          data.forEach(
            (stu) =>
              (initialData[stu.student_id] = stu.attendance_status ?? "present")
          );
  ...
  const submitAttendance = () => {
    ...
    fetch(
      "http://localhost:8888/management_attendance/attendance-tracker/backend/api/teacher/submitAttendance.php",
      {
        method: "POST",
        credentials: "include",
      ...
      .then((data) => {
        setMessage(
          data.success
            ? "Attendance saved."
            : data.error || "Error saving attendance."
        );
  ...
  return (
    <div className="mark-attendance-page">
      ...
      <div className="mark-attendance-card">
        ...
        <div className="select-container">
          <div className="selection">
            ...
            >
              ...
              {courses.length > 0 ? (
                courses.map((c, index) => (
                  <option key={index} value={c.course_id}>
                    {c.course_name}
                  </option>
                ))
              ) : (
            ...
            <div className="selection">
              ...
              >
                ...
                {sessions.map((s, index) => (
                  <option key={index} value={s.session_id}>
                    {s.session_date}
                  </option>
                ))}
              </select>
                  ...
                  <tr key={stu.student_id}>
                    ...
                    <td>
                      <input
                        ...
                        onChange={() =>
                          setStudentAttendance(stu.student_id, "present")
                        }
                      />
                    </td>
                    <td>
                      <input
                        ...
                        onChange={() =>
                          setStudentAttendance(stu.student_id, "absent")
                        }
                      />
                    </td>
                    <td>
                      <input
                        ...
                        onChange={() =>
                          setStudentAttendance(stu.student_id, "excused")
                        }
                      />
                    </td>
                  </tr>
        ...
        {students.length > 0 && (
          <button className="save-button" onClick={submitAttendance}>
            Save Attendance
          </button>
        )}
