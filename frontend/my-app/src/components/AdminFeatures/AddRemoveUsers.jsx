...
const Users = () => {
  ...
  const fetchUsers = async () => {
    const res = await fetch(`${BASE_URL}/getUsers.php`, { credentials: 'include' });
  ...
  const handleAddUser = async () => {
    ...
    if (!newUser.username.trim() || !newUser.user_email.trim() || !newUser.password.trim()) {
      alert("Username, Email and Password are required");
      return;
    }

    if ((newUser.user_role === "teacher" || newUser.user_role === "student") && !newUser.first_name.trim()) {
        ...
        <div className="users-section">
          ...
          {expandAdd && (
            <div className="users-form">
              ...
              <input name="username" value={newUser.username} placeholder="Username" onChange={handleChange} />
              <input name="user_email" value={newUser.user_email} placeholder="Email" onChange={handleChange} />
              <input name="password" value={newUser.password} type="password" placeholder="Password" onChange={handleChange} />

              <select name="user_role" value={newUser.user_role} onChange={handleChange}>
                <option value="student">Student</option>
                <option value="teacher">Teacher</option>
                <option value="admin">Admin</option>
              </select>

              {(newUser.user_role === "student" || newUser.user_role === "teacher") && (
                <>
                  <input name="first_name" value={newUser.first_name} placeholder="First Name" onChange={handleChange} />
                  <input name="last_name" value={newUser.last_name} placeholder="Last Name" onChange={handleChange} />
                </>
              )}

              {newUser.user_role === "student" && (
                <select name="group_id" value={newUser.group_id} onChange={handleChange}>
              ...
              {newUser.user_role === "teacher" && (
                <select name="department_id" value={newUser.department_id} onChange={handleChange}>
            ...
            <ul className="users-delete-list">
              {users.map(u => (
                <li key={u.user_id}>
                  ...
                  <button className="delete-btn" onClick={() => handleDeleteUser(u.user_id)}>
                    Delete
                  </button>
                </li>
              ))}
